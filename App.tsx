...
function App() {
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''} font-sans h-screen flex flex-col bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 transition-colors duration-200 overflow-hidden`}>
        ...
        {statusNotification && (
            <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[2000] animate-fade-in-down px-4 w-full max-w-md">
                <div className="bg-amber-100 dark:bg-amber-900/90 border border-amber-300 dark:border-amber-700 text-amber-900 dark:text-amber-100 px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 backdrop-blur-sm">
                    ...
                    <button onClick={() => setStatusNotification(null)} className="ml-auto text-amber-600 dark:text-amber-300 p-1 rounded-full"><svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button>
                </div>
            </div>
        )}
            ...
            <div className={`flex flex-col gap-0 bg-white dark:bg-gray-800 xl:rounded-lg xl:border border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-200 ${mobileView === 'editor' ? 'flex flex-1' : 'hidden xl:flex'} h-full overflow-y-auto`}>
                ...
                <div className="flex flex-shrink-0 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 justify-between items-center pr-2">
                    <div className="flex flex-1">
                        ...
                        <ToolTabButton label={mode === 'image' ? 'Image Attributes' : (mode === 'video' ? 'Video Attributes' : 'Story Attributes')} tab="attributes" current={activeToolTab} />
                    </div>
                </div>
                <div className="relative bg-gray-50/30 dark:bg-gray-900/30 flex-1 overflow-hidden flex flex-col min-h-[450px] pb-[3.5rem] xl:pb-0">
                    {(Object.keys(pendingClarificationAnswers).length + Object.keys(pendingAttributeUpdates).length + Object.keys(pendingRelationshipUpdates).length) > 0 && (
                        <div className="flex-shrink-0 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800 p-3 flex justify-between items-center animate-fade-in z-20">
                            <span className="text-xs text-blue-800 dark:text-blue-200 font-medium">Pending changes...</span>
                            <button onClick={handleApplyAllUpdates} disabled={isUpdatingPrompt} className={`bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-1.5 px-4 rounded-md shadow-sm transition-all ${isUpdatingPrompt ? 'opacity-70 cursor-wait' : ''}`}>
        ...
        <div className="xl:hidden bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex justify-around p-2 z-[200] fixed bottom-0 left-0 right-0" style={{ paddingBottom: 'max(0.5rem, env(safe-area-inset-bottom))' }}>
            <button onClick={() => setMobileView('editor')} className={`flex-1 flex flex-col items-center py-1 transition-colors ${mobileView === 'editor' ? 'text-blue-600' : 'text-gray-500'}`}><svg className="h-5 w-5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg><span className="text-[10px] font-bold">EDITOR</span></button>
            <button onClick={() => setMobileView('preview')} className={`flex-1 flex flex-col items-center py-1 transition-colors ${mobileView === 'preview' ? 'text-blue-600' : 'text-gray-500'}`}><svg className="h-5 w-5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg><span className="text-[10px] font-bold">PREVIEW</span></button>
        </div>

        {showInfoModal && (
          <div className="fixed inset-0 z-[3000] bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowInfoModal(false)}>
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-lg w-full p-6 relative border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">Explore the power of collaborative prompting with multi-provider support.</p>
              <button onClick={() => setShowInfoModal(false)} className="bg-blue-600 text-white px-6 py-2 rounded-lg w-full font-bold">Close</button>
            </div>
          </div>
        )}
        ...
        {showModeChangePopup && (
            <div className="fixed bottom-4 right-4 z-[2000] animate-fade-in-up max-w-sm w-full mx-auto px-4 sm:px-0">
                <div className="bg-white dark:bg-gray-800 border-l-4 border-amber-400 rounded-r shadow-2xl p-4 relative border border-gray-200 dark:border-gray-700">
                    <button onClick={() => setShowModeChangePopup(false)} className="absolute top-2 right-2 text-gray-400 hover:text-gray-600">×</button>
                    <h3 className="text-sm font-bold">Mode Changed</h3>
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">Previous data is preserved for reference.</p>
                </div>
            </div>
        )}
    </div>
